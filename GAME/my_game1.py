...
def draw_window(red, yellow, red_bullets, yellow_bullets, red_health, yellow_health):
    WID.blit(SPACE, (0, 0))
    pygame.draw.rect(WID, BLACK, BORDER)
    ...
    WID.blit(yellow_health_text, (10, 10))
    WID.blit(red_health_text, (WIDTH - red_health_text.get_width() - 10, 10))

    WID.blit(YELLOW_SPACESHIP, (yellow.x, yellow.y))
    WID.blit(RED_SPACESHIP, (red.x, red.y))

    for bullet in red_bullets:
        pygame.draw.rect(WID, RED, bullet)
    for bullet in yellow_bullets:
        pygame.draw.rect(WID, YELLOW, bullet)
...
def draw_winner(text):
    ...
    WID.blit(draw_text, (WIDTH/2 - draw_text.get_width()/2, HEIGHT/2 - draw_text.get_height()/2))
...
def main():
    ...
    while run:
        ...
        winner_test = ''
        if red_health <= 0:
            winner_test = 'yellow win'
        if yellow_health <= 0:
            winner_test = 'red win'
        if winner_test != '':
            draw_winner(winner_test)
            break
